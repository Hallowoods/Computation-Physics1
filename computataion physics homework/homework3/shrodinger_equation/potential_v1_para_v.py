...
# -*- coding: utf-8 -*-
'''
@File    :   potential_v1_para_v.py
@Time    :   2023/09/28 11:47:07
@Author  :   Zhongyi Yang 
@Version :   1.0
@Desc    :   None
'''

# 使用变分法求解一维薛定谔方程，势场为 V(x)=x^2,以中心的高斯波包（不同宽度）为一组基展开
import math
from sympy import *
import numpy as np
...
def Phi_H_Phi(vi, vj):        #哈密顿量的矩阵元
    return math.sqrt(vi * vj) * (2 * h_bar**2 * vi * vj + 2 * m * s**2 * (vi + vj) + m)/ (2 * m * (vi + vj)**(3 / 2) * math.sqrt(math.pi))
...
fig, ax = plt.subplots()
for i in range(3):
    Wavefunction = [wave_function(indexes[i],v_list,eigenvectors,xi) for xi in x_list]
